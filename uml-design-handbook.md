...
获取业务用例的过程是通过访谈业务主角、角色扮演获取、不同角色视角、角色扮演提炼的，可以根据业务用例划分工作量。（工作量估算方法：WBS-WorkBown Structure,工作包估算法：经验法、Delphi发，Loc代码行估算法）
...
| 编号  | 名称 | 说明           | 期望                                                         |
| ----- | ---- | -------------- | ------------------------------------------------------------ |
| SH001 | 用户 | 主要的使用用户 | 通过计算机获取一个概念图<br>编辑概念图，反向查阅基础资料<br>按照概念图的基础上，对一个概念做出简短的总结 |
...
边界-业务目标1：自动分析文章的概念内容,并能管理、创建知识内容。
...
我希望系统能帮我快递阅读一个资料的时候，有的时候是一个网页博客，有的时候是一个百度百科，wiki的词条，如果是wiki的词条，系统能帮我建立概念的知识图谱。

2、打算在这个系统里面做些什么事

我想要输入一个百科/wiki的词条的时候，系统显示爬取多个链接，并以链接为中心词，画出图谱，如果词出现的多，那么这个词就大一点，图谱的关系，我可以像思维导图一样开进行编辑。如果是一个普通的阅读材料，那么系统能做一个分词，这个时候，只要一个词云就可以了，同样的，我也可以对这个词云进行编辑成思维导图。我同样可以自由的分类、命名这些图谱，自动跳转到原来的资料。我还需要把图谱之间做上自定义的关联关系。再获取资料的基础上，我可以对每一个资料进总结。自己编辑、创建知识图谱，或者知识条目，形成一个知识的网络。

3、做这件的事情是为了达到什么目的？

有的时候，我需要快速的阅读一篇文章，又怕漏掉什么重要的概念，这个软件可以帮助我减轻认知负担和记忆负担。
